...
impl Db {
    ...
    pub fn ensure_indexes(&self, _spec: &DbSpec) -> Result<()> {
        Ok(())
    }

    pub fn upsert_from_meta(
        &self,
        spec: &DbSpec,
        branch: &str,
        meta: &JsonValue,
        ...
        if spec.engine.to_lowercase() != "polodb" {
            return Err(anyhow!("unsupported engine: {}", spec.engine));
        }
        let now: DateTime<Utc> = Utc::now();
        let mut doc_json = serde_json::Map::new();
        // mapping
        for m in &spec.mapping {
            let val = if let Some(der) = &m.derive {
                self.eval_derive(der, meta)?
            } else if let Some(from) = &m.from {
                self.extract_jsonpath(meta, from)
            } else {
                JsonValue::Null
            };
        ...
        // system fields
        doc_json.insert(
            SYS_BRANCH.to_string(),
            JsonValue::String(branch.to_string()),
        );
        doc_json.insert(
            SYS_META_DIR.to_string(),
            JsonValue::String(meta_dir.to_string()),
        );
        let ts = JsonValue::String(now.to_rfc3339());
        if !doc_json.contains_key(SYS_CREATED_AT) {
            doc_json.insert(SYS_CREATED_AT.to_string(), ts.clone());
        }
    ...
    pub fn query(
        ...
        if let Some(b) = branch {
            if b != "all" {
                filter_doc.insert(SYS_BRANCH, b);
            }
        }
        if let Some(f) = &params.filter {
            self.apply_filter(&mut filter_doc, f, spec)?;
        }

        // Pagination
        let def_page_size = spec
            .query_policy
            .as_ref()
            .map(|p| p.pagination.defaultPageSize)
            .unwrap_or(25);
        let max_page_size = spec
            .query_policy
            .as_ref()
            .map(|p| p.pagination.maxPageSize)
            .unwrap_or(250);
        ...
        let sorts = params
            .sort
            .as_ref()
            .or_else(|| spec.query_policy.as_ref().map(|p| &p.sort))
            .cloned()
            .unwrap_or_default();
        ...
        if !sort_doc.is_empty() {
            let mut sort_fields: Vec<(String, i32)> = sort_doc
                .into_iter()
                .map(|(k, v)| (k, v.as_i32().unwrap_or(1)))
                .collect();
    ...
    fn eval_derive(&self, d: &DeriveSpec, meta: &JsonValue) -> Result<JsonValue> {
        let fname = if let Some(alias) = &d.fn_alias {
            if !alias.is_empty() {
                alias
            } else {
                &d.fn_
    ...
    fn arg_as_string(&self, arg: Option<&DeriveArg>, meta: &JsonValue) -> Result<String> {
        ...
        if let Some(ref from) = a.from {
            Ok(self
                .extract_jsonpath(meta, from)
                .as_str()
                .unwrap_or("")
                .to_string())
        } else if let Some(ref v) = a.value {
...
impl Db {
    fn apply_filter(
        &self,
        out: &mut bson::Document,
        filter: &JsonValue,
        spec: &DbSpec,
    ) -> Result<()> {
        let qp = if let Some(q) = &spec.query_policy {
            q
        } else {
            return Ok(());
        };
    ...
    fn push_op(
        &self,
        sub: &mut bson::Document,
        op: &str,
        val: &JsonValue,
        allowed_ops: &Vec<String>,
    ) {
        let allow = |name: &str| {
            allowed_ops.is_empty() || allowed_ops.iter().any(|o| o.eq_ignore_ascii_case(name))
        };
        match op.to_ascii_lowercase().as_str() {
            ...
            "gte" if allow("gte") => {
                sub.insert("$gte", json_to_bson(val.clone()));
            }
            "lte" if allow("lte") => {
                sub.insert("$lte", json_to_bson(val.clone()));
            }

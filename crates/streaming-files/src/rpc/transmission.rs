...
impl TransmissionClient {
    ...
    async fn rpc<T: for<'de> Deserialize<'de>, A: Serialize + ?Sized>(
        &self,
        method: &str,
        args: &A,
    ) -> Result<T> {
        #[derive(Serialize)]
        struct Req<'a, A: Serialize + ?Sized> {
            method: &'a str,
            arguments: &'a A,
        }
        #[derive(Deserialize)]
        ...
        let req1 = self.client.post(&self.base_url).json(&Req {
            method,
            arguments: args,
        });
        let mut resp = req1.send().await.map_err(|e| {
            StreamingError::RpcUnavailable(format!("transmission request failed: {}", e))
        })?;
        if resp.status().as_u16() == 409 {
            ...
            if let Some(h) = resp.headers().get("X-Transmission-Session-Id").cloned() {
                if let Ok(s) = header_to_string(&h) {
                    ...
                    resp = req2.send().await.map_err(|e| {
                        StreamingError::RpcUnavailable(format!(
        ...
        let body = resp
            .json::<Resp<T>>()
            .await
            .map_err(|e| StreamingError::RpcUnavailable(format!("transmission decode: {}", e)))?;
        ...
        body.arguments.ok_or_else(|| {
            StreamingError::RpcUnavailable("missing arguments in transmission response".into())
        })
    }
}
...
impl TorrentClient for TransmissionClient {
    ...
    async fn add_magnet(
        ...
        struct Out {
            ...
            torrent_added: Option<Added>,
            #[serde(rename = "torrent-duplicate")]
            torrent_duplicate: Option<Added>,
        }
        #[derive(Deserialize)]
        struct Added {
            #[serde(rename = "hashString")]
        ...
        if let Some(a) = added {
            Ok(AddResult {
                info_hash: a.hash_string.unwrap_or_default(),
                name: a.name,
            })
        } else {
            Ok(AddResult {
    ...
    async fn list_files(&self, info_hash: &str) -> Result<Vec<TorrentFile>> {
        ...
        let args = Args {
            fields: vec!["hashString", "files", "fileStats"],
            ids: vec![info_hash],
        };
        let out: TorrentGetResp = self.rpc("torrent-get", &args).await?;
        if let Some(t) = out.torrents.into_iter().next() {

...
fn zero_length_never_playable() {
    assert!(!is_playable_by_thresholds(0, 0, 16 * 1024 * 1024, 64 * 1024 * 1024, 1));
}
...
fn first_bytes_rule_allows() {
    ...
    assert!(is_playable_by_thresholds(min_first - 1, len, min_first, 64 * 1024 * 1024, 1));
    // downloaded at first-bytes threshold
    assert!(is_playable_by_thresholds(min_first, len, min_first, 64 * 1024 * 1024, 1));
}
...
fn total_bytes_vs_percent_min_logic() {
    ...
    assert!(!is_playable_by_thresholds(15 * 1024 * 1024, len, 16 * 1024 * 1024, min_total, percent));
    // 63MB: above first-bytes (16MB) so playable even if below 64MB total
    assert!(is_playable_by_thresholds(63 * 1024 * 1024, len, 16 * 1024 * 1024, min_total, percent));
    // 64MB: at min_total -> playable
    assert!(is_playable_by_thresholds(64 * 1024 * 1024, len, 16 * 1024 * 1024, min_total, percent));
}
...
fn tiny_file_uses_file_size_caps() {
    ...
    assert!(!is_playable_by_thresholds(50 * 1024, len, min_first, min_total, percent));
    // At ~52KB should be allowed by percent rule
    assert!(is_playable_by_thresholds(53 * 1024, len, min_first, min_total, percent));
}

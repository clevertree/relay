...
fn handle_host(cmd: HostCmd, paths: ConfigPaths) -> Result<()> {
    match cmd {
        HostCmd::Start { port, root, git_port } => {
            ...
            let repos_root = root_path.join("repos");
            let static_root = root_path.join("static");
            ...
            if !static_root.exists() {
                std::fs::create_dir_all(&static_root).with_context(|| format!(
                    "Failed to create static root at {}",
                    static_root.display()
                ))?;
            }

            println!(
                "Starting host on http://0.0.0.0:{} with root {}",
                port, root
            );
            ...
            // Start HTTP server (tiny_http) in a background thread
            use tiny_http::{Server, Response, Method, Request, StatusCode, Header};
            let http_addr = format!("0.0.0.0:{}", port);
            let server = Server::http(&http_addr).with_context(|| format!("Failed to bind HTTP on {}", http_addr))?;
            let repos_root_clone = base_path.clone();
            let static_root_clone = std::fs::canonicalize(&static_root).unwrap_or(static_root.clone());
            let (stop_tx, stop_rx) = std::sync::mpsc::channel::<()>();

            let http_thread = std::thread::spawn(move || {
                loop {
                    // We can't easily select on Ctrl+C, but we can use non-blocking try_recv to exit idle
                    if let Ok(_) = stop_rx.try_recv() {
                        break;
                    }
                    let req = match server.recv() { Ok(r) => r, Err(_) => { continue; } };
                    if let Err(e) = handle_http(req, &repos_root_clone, &static_root_clone) {
                        eprintln!("HTTP handler error: {}", e);
                    }
                }
            });

            // Block until Ctrl+C, then stop git daemon and HTTP server gracefully
            ...
            // Stop HTTP thread
            let _ = stop_tx.send(());
            // Best-effort: poke server by making a local request to unblock
...
use anyhow::anyhow;
fn handle_http(req: tiny_http::Request, repos_root: &Path, static_root: &Path) -> Result<()> {
    let method = req.method().clone();
    let url = req.url().to_string(); // includes path and query
    let (path, query) = split_path_query(&url);

    // Route matching
    match (method, path.as_str()) {
        // Static UI export
        (tiny_http::Method::Get, p) if p == "/" || p.starts_with("/static/") || p.ends_with(".html") => {
            return serve_static(req, static_root, &path);
        }
        // Repos JSON listing at /repos
        (tiny_http::Method::Get, "/repos") => {
            return api_list_repos(req, repos_root);
        }
        // Serve raw files directly under /repos/<name>/...
        (tiny_http::Method::Get, p) if p.starts_with("/repos/") => {
            return serve_repo_file_or_dir(req, repos_root, &path);
        }
        _ => {
            let mut resp = tiny_http::Response::from_string("Not Found");
            *resp.status_mut() = tiny_http::StatusCode(404);
...
fn split_path_query(url: &str) -> (String, String) {
    if let Some((p, q)) = url.split_once('?') { (p.to_string(), q.to_string()) } else { (url.to_string(), String::new()) }
}
...
fn parse_query(q: &str) -> std::collections::HashMap<String, String> {
    ...
    for pair in q.split('&') {
        ...
        if !k.is_empty() { map.insert(percent_decode(k), percent_decode(v)); }
    }
    map
}

fn percent_decode(s: &str) -> String {
    // naive percent-decoder for common cases
    let bytes = percent_encoding::percent_decode_str(s).decode_utf8_lossy().to_string();
    bytes
}

fn extract_repo_name<'a>(path: &'a str, prefix: &str, suffix: &str) -> Result<String> {
    if let Some(mid) = path.strip_prefix(prefix) {
        if let Some(name) = mid.strip_suffix(suffix) {
            let name = name.trim_matches('/');
            if name.is_empty() { anyhow::bail!("missing repo name"); }
            return Ok(name.to_string());
        }
    }
    anyhow::bail!("bad repo path")
}

fn serve_static(req: tiny_http::Request, static_root: &Path, path: &str) -> Result<()> {
    let rel = if path == "/" { PathBuf::from("index.html") } else { PathBuf::from(path.trim_start_matches('/').trim_start_matches("static/")) };
    ...
    let len = file.metadata().ok().and_then(|m| Some(m.len())).unwrap_or(0);
    let mut resp = tiny_http::Response::from_file(file);
    resp.add_header(tiny_http::Header::from_bytes(&b"Content-Type"[..], mime.as_bytes()).unwrap());
    resp.add_header(tiny_http::Header::from_bytes(&b"Cache-Control"[..], b"public, max-age=60").unwrap());
    resp.add_header(tiny_http::Header::from_bytes(&b"Content-Length"[..], len.to_string().as_bytes()).unwrap());
    let _ = req.respond(resp);
    Ok(())
}

fn api_list_repos(req: tiny_http::Request, repos_root: &Path) -> Result<()> {
    #[derive(serde::Serialize)]
    struct RepoItem { name: String, path: String }
    let mut items: Vec<RepoItem> = Vec::new();
    if repos_root.is_dir() {
        for e in std::fs::read_dir(repos_root)? { let e = e?; if e.file_type()?.is_dir() {
            let name = e.file_name().to_string_lossy().to_string();
            items.push(RepoItem { name: name.clone(), path: format!("/api/repos/{}/tree", name) });
        }}
    }
    let body = serde_json::to_string(&items)?;
    let mut resp = tiny_http::Response::from_string(body);
    resp.add_header(tiny_http::Header::from_bytes(&b"Content-Type"[..], &b"application/json; charset=utf-8"[..]).unwrap());
    let _ = req.respond(resp);
    Ok(())
}

// Serve raw files under /repos/<name>/path...  (no server-side allowlist; client enforces)
fn serve_repo_file_or_dir(req: tiny_http::Request, repos_root: &Path, request_path: &str) -> Result<()> {
    // request_path looks like /repos/<name>/optional/subpath
    let trimmed = request_path.trim_start_matches('/');
    let mut parts = trimmed.splitn(3, '/');
    let first = parts.next(); // "repos"
    let name = parts.next();  // repo name
    let tail = parts.next().unwrap_or(""); // remaining path (may be empty)
    if first != Some("repos") || name.is_none() {
        return respond_status(req, 400, "bad repos path");
    }
    let name = name.unwrap();
    let repo_dir = repos_root.join(name);
    if !repo_dir.is_dir() { return respond_status(req, 404, "repo not found"); }

    // Join safely within repo
    let target = normalize_join(&repo_dir, Path::new(tail)).ok_or_else(|| anyhow!("bad path"))?;
    if !target.starts_with(&repo_dir) { return respond_status(req, 400, "invalid path"); }

    if target.is_dir() {
        // JSON directory listing: [{ name, type, size }]
        #[derive(serde::Serialize)]
        struct Entry { name: String, r#type: String, size: Option<u64> }
        ...
        for ent in std::fs::read_dir(&target)? {
            ...
            if ft.is_dir() {
                entries.push(Entry { name, r#type: "dir".into(), size: None });
            } else if ft.is_file() {
                let size = ent.metadata().ok().map(|m| m.len());
                entries.push(Entry { name, r#type: "file".into(), size });
            }
        }
        #[derive(serde::Serialize)]
        struct Payload { entries: Vec<Entry> }
        let body = serde_json::to_string(&Payload { entries })?;
        let mut resp = tiny_http::Response::from_string(body);
        resp.add_header(tiny_http::Header::from_bytes(&b"Content-Type"[..], &b"application/json; charset=utf-8"[..]).unwrap());
        let _ = req.respond(resp);
        return Ok(());
    }
    if !target.exists() || !target.is_file() { return respond_status(req, 404, "not found"); }

    // Guess MIME by extension for convenience
...
fn respond_status(req: tiny_http::Request, code: u16, msg: &str) -> Result<()> {
    let mut resp = tiny_http::Response::from_string(msg.to_string());
    *resp.status_mut() = tiny_http::StatusCode(code);
    let _ = req.respond(resp);
    Ok(())
}

// Reuse normalize_join from relay-core-like logic for server as well
fn normalize_join(base: &Path, rel: &Path) -> Option<PathBuf> {
    let mut parts: Vec<&str> = Vec::new();
    for comp in rel.components() {
        use std::path::Component::*;
        match comp {
            tiny_http::RequestUri::AbsolutePath(_) => {},

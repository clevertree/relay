...
const DEFAULTS = {
    ...
    recentRepos: ['http://localhost:8080'],
...
if (browser.tabs && browser.tabs.onRemoved) {
    browser.tabs.onRemoved.addListener((tabId) => {
        ...
        try {
            removeBranchHeaderRule(tabId);
        } catch (_) {
        }
        try {
            removeRepoHeaderRule(tabId);
        } catch (_) {
        }
    });
}
...
function removeBranchHeaderRule(tabId) {
    ...
    if (rec.ruleId && isChromium() && chrome.declarativeNetRequest) {
        try {
            chrome.declarativeNetRequest.updateSessionRules({removeRuleIds: [rec.ruleId], addRules: []}, () => {
            });
        } catch {
        }
    }
    if (rec.listener && browser.webRequest && browser.webRequest.onBeforeSendHeaders) {
        try {
            browser.webRequest.onBeforeSendHeaders.removeListener(rec.listener);
        } catch {
        }
    }
...
async function qbtFiles(base, hash) {
    ...
    return (files || []).map((f, i) => ({
        ...
        path: f.name,
        length: f.size,
        downloaded: f.progress * f.size,
        is_media: /\.(mp4|mkv|avi|mov|webm)$/i.test(f.name)
    }));
}
    ...
    switch (method) {
        ...
        case 'status': {
            ...
            if (cfg?.qbt?.base) {
                ...
                return {
                    state: info.state || info.state_enum || 'downloading',
                    downloaded: Math.round(info.downloaded || info.completed || info.progress * info.size || 0),
                    size: info.size || 0
                };
            }
            return {error: 'no backend configured'};
        ...
        case 'requestPlay': {
            ...
            if (cfg?.qbt?.base) {
                ...
                return {
                    allow: ratio >= 0.01,
                    reason: ratio >= 0.01 ? undefined : 'not enough downloaded',
                    path: pick.path
                };
            }
...
async function listManagedTorrents() {
    ...
    if (cfg?.trans?.base) {
        try {
            const body = {
                method: 'torrent-get',
                arguments: {fields: ['id', 'hashString', 'name', 'percentDone', 'status']}
            };
            // One round to get session id if needed
            let sid = '';
            let r = await fetch(cfg.trans.base, {
                method: 'POST',
    ...
    (async () => {
        switch (msg?.type) {
            ...
            case 'pageBranch': {
                ...
                if (tabId != null) {
                    const info = {
                        origin: msg.origin || (sender?.origin || null),
            ...
            case 'setActiveTabBranch': {
                ...
                try {
                    ...
                    try {
                        await addBranchHeaderRule(tab.id, origin, branch);
                    } catch {
                    }
            ...
            case 'setActiveTabRepo': {
                ...
                try {
                    ...
                    try {
                        await addRepoHeaderRule(tab.id, origin, repo);
                    } catch {
                    }
                    await new Promise((resolve) => browser.tabs.reload(tab.id, {}, resolve));
                    sendResponse({ok: true});
